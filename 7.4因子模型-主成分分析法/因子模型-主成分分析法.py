...
def get_positions(df_window, topN=topN):
    ...
    # 例: argsort([3,1,2]) -> [1,2,0] (返回数组升序排序后，各数在原数组中的索引)
    # sort_idx的最后topN个数就是r_next最大的topN个股票的index
    # 把positions向量和r_next向量的索引对应起来
...
for i in range(0, len(df)-lookback):
    ...
    positions = get_positions(df_window)  # for the next day
#    exposure = np.abs(positions).sum()  # = 100
    
    next_return = daily_return.iloc[lookback+i]  # 下一天的真实收益率  
